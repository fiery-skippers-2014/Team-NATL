  ...
  class Client
    ...
    def add_to_database(photos)
      photos.each_with_index do |photo,index|
        ...
        selfie_city = City.find_by_name('Selfie City')
        # p new_city
        if new_city == nil
          city_added_id = selfie_city.id
        else
          city_added_id = new_city.id
        end
        p city_added_id
        if Photo.find_by_insta_id(photo[0])
          p 'already exists'
        else
          @photo = Photo.create(insta_id: photo[1][:insta_id], latitude: photo[1][:latitude], longitude: photo[1][:longitude], insta_text: photo[1][:image_text], url: photo[1][:image_url], city_id: city_added_id )
        end
      end
    end
